...
class Space:
    ...
    def hop(self):
        ...
        """
        # irradiate
        lengths = np.random.exponential(1.0 / self.mu, self.n)
        ...
        absorbed = np.logical_or(
            np.random.rand(self.n) < self.absorb_ratio,
            (new_positions**2).sum(axis=1) > max_distance**2,
        )
